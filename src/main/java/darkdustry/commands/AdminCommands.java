...
public class AdminCommands {
    public static void load() {
        ...
        register("core", (args, player) -> {
            ...
            Bundle.send(player, player.blockOn() == core ? "commands.core.success" : "commands.core.failed", Icons.icon(core), core.name, coloredTeam(team));
        });
        ...
        register("unit", (args, player) -> {
            ...
            target.unit(unit);
            Bundle.send(target, "commands.unit.success", Icons.icon(type), type.name);
            if (target != player) Bundle.send(player, "commands.unit.success.player", target.coloredName(), Icons.icon(type), type.name);
        });
        register("effect", (args, player) -> {
            ...
            Bundle.send(target, duration > 0 ? "commands.effect.apply.success" : "commands.effect.remove.success", Icons.icon(effect), effect.name, duration);
            if (target != player)
                Bundle.send(player, duration > 0 ? "commands.effect.apply.success.player" : "commands.effect.remove.success.player", target.coloredName(), Icons.icon(effect), effect.name, duration);
        });
        register("give", (args, player) -> {
            ...
            Bundle.send(player, "commands.give.success", amount, Icons.icon(item), item.name, coloredTeam(team));
        });
        register("spawn", (args, player) -> {
            ...
            Bundle.send(player, "commands.spawn.success", amount, Icons.icon(type), type.name, coloredTeam(team));
        });
        ...
        register("fill", (args, player) -> {
            ...
            Bundle.send(player, "commands.fill.success", width, height, Icons.icon(block), block.name);
        });
    }
