...
public enum DiscordCommands implements CommandRunner<MessageContext> {
    ...
    maps("Список всех карт сервера.", "[страница]", (args, context) -> {
        if (notPageDs(context, args)) return;
        if (Vars.maps.customMaps().isEmpty()) {
            context.info(":map: На сервере нет карт.");
            return;
        }

        int page = args.length > 0 ? parseInt(args[0]) : 1, pages = Mathf.ceil(Vars.maps.customMaps().size / 16f);
        if (notPageDs(context, page, pages)) return;

        StringBuilder maps = new StringBuilder();
        Seq<Map> list = Vars.maps.customMaps();
        for (int i = 16 * (page - 1); i < Math.min(16 * page, list.size); i++) {
            maps.append("**").append(i).append(".** ").append(stripColors(list.get(i).name())).append("\n");
        }
