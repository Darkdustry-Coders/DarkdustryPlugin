...
public class BuildSelectListener {
    ...
    public static void call(final BuildSelectEvent event) {
        if (config.alertsEnabled() && !event.breaking && event.builder != null && event.builder.buildPlan() != null && dangerousBuildBlocks.contains(event.builder.buildPlan().block) && event.team.cores().contains(c -> event.tile.dst(c) < alertsDistance) && interval.get(alertsTimer)) {
