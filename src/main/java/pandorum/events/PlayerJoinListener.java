    ...
    public static void call(final EventType.PlayerJoin event) {
        ...
        PlayerModel.find(new BasicDBObject("UUID", event.player.uuid()), playerInfo -> {
            if (playerInfo.hellomsg) {
                ...
                Call.menu(event.player.con, 0, Bundle.format("events.hellomsg.header", findLocale(event.player.locale)), Bundle.format("events.hellomsg", findLocale(event.player.locale), "discord.gg/xagFsu9K5W"), options);
            }
        });
