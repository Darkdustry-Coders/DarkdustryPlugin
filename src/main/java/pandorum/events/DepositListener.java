...
public class DepositListener {
    ...
    public static void call(final DepositEvent event) {
        if (config.alertsEnabled() && event.tile.block instanceof NuclearReactor && event.item.explosiveness > 0f && event.player.team().cores().contains(c -> event.tile.dst(c.x, c.y) < config.alertsDistance)) {
