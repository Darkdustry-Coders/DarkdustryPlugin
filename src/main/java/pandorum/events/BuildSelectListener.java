...
public class BuildSelectListener {
    ...
    public static void call(final EventType.BuildSelectEvent event) {
        if (state.rules.reactorExplosions && PandorumPlugin.config.alertsEnabled() && !event.breaking && event.builder != null && event.builder.buildPlan() != null && event.builder.buildPlan().block instanceof NuclearReactor && event.builder.isPlayer() && event.team.cores().contains(c -> event.tile.dst(c.x, c.y) < PandorumPlugin.config.alertsDistance) && PandorumPlugin.interval.get(0, 900f)) {
