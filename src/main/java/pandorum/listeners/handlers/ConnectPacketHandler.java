...
public class ConnectPacketHandler implements Cons2<NetConnection, ConnectPacket> {
    ...
    public void get(NetConnection con, ConnectPacket packet) {
        ...
        if (extraMods.any() || missingMods.any()) {
            ...
            if (extraMods.any())
                reason += Bundle.format("kick.unnecessary-mods", findLocale(locale), extraMods.toString("\n> "));
            if (missingMods.any())
                reason += Bundle.format("kick.missing-mods", findLocale(locale), missingMods.toString("\n> "));
